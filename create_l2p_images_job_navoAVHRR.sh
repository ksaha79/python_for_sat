...
#Dat_Dir=/srv/pfv53_l3c
 Dat_Dir=/nodc/web/data.nodc/htdocs/ghrsst/GDS2/L2P/AVHRRMTB_G/NAVO/v1
 Out_Dir=$THISDIR/L2P_IMAGES
 Log_Dir=$THISDIR/LOG
...
#                    >& $ldir1/image_pfv5.3_l2p_${varName}_J${jj}_$cid.log &
          taskset -c $pcid $THISDIR/image_navoAVHRR_l2p_$varName.py $filelist $outfile_day 0 \
                    >& $ldir1/image_navoAVHRR_l2p_${varName}_J${jj}_$cid.log &
          pid=$!
          set_cpu_busy $cid $pid
          taskset -c $pcid $THISDIR/image_navoAVHRR_l2p_$varName.py $filelist $outfile_night 1 \
                    >> $ldir1/image_navoAVHRR_l2p_${varName}_J${jj}_$cid.log &
          pid=$!
          set_cpu_busy $cid $pid
       done
