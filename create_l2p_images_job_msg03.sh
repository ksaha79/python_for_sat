...
#Dat_Dir=/srv/pfv53_l3c
 Dat_Dir=/nodc/web/data.nodc/htdocs/ghrsst/GDS2/L2P/MSG03/OSPO/v1
 Out_Dir=$THISDIR/L2P_IMAGES/MSG
 Log_Dir=$THISDIR/LOG/MSG
...
#   filenm="$(date -d "$year-01-01 + $((j-1)) days" +%Y%m%d)"
    filenm="20160101001500-OSPO-L2P_GHRSST-SSTskin-MSG03-v02.0-"
...
#                      aerosol_dynamic_indicator sea_ice_fraction wind_speed; do
     for varName in sea_surface_temperature; do
          outfile=$odir1/$(basename $infile .nc)_$varName.png
          free_cpu_id cid
          physical_cpu $cid pcid
#          echo taskset -c $pcid $THISDIR/image_pfv5.3_l2p_$varName.py $filelist $outfile
          taskset -c $pcid $THISDIR/image_MSG03_l2p_$varName.py -i $infile -o $outfile \
                    >& $ldir1/image_MSG03_l2p_${varName}_J${jj}_$cid.log &
          pid=$!
