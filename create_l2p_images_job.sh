...
#Dat_Dir=/srv/pfv53_l3c
 Dat_Dir=/nodc/projects/satdata.5/Converters/Data_PFV53/PFV53_NC_L2P
 ...
 [ -d $ldir1 ] || mkdir -p $ldir1
    jj=year*1000+j
	ff=0
    for infile in $ddir1/*_G_${jj}_*.nc; do
...
#                    >& $ldir1/image_pfv5.3_l2p_${varName}_J${jj}_$cid.log &
          taskset -c $pcid $THISDIR/image_pfv5.3_l2p_$varName.py $filelist $outfile_day 0 \
                    >& $ldir1/image_pfv5.3_l2p_${varName}_J${jj}_$cid.log &
	  taskset -c $pcid $THISDIR/image_pfv5.3_l2p_$varName.py $filelist $outfile_night 1 \
                    >& $ldir1/image_pfv5.3_l2p_${varName}_J${jj}_$cid.log &
          pid=$!
